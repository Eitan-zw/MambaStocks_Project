...
parser.add_argument('--ts-code', type=str, default='12345',
                    help='Stock code')                    
...
amount_change = pd.DataFrame(np.diff(data["Close"]))
amount_change = np.array(amount_change).reshape(-1)
close = np.array(data["Close"])
close = close[1:]
ratechg = np.divide(amount_change, close)
...
trainX, testX = dat[1:-args.n_test, :], dat[-args.n_test:, :]
